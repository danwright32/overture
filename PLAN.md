...
The original plan also named a second arm for the offer: state the rate, or point at a page describing it. **That arm is dropped (#612):** the site has no such page, so the only way to write that variant is to invent a URL that 404s in an email Dan actually sends. Left standing it read like a working feature, while actually being a live instruction to the drafter to fabricate a link. (The other arm went too: since #1906 a cold pitch states no rate at all. See Pricing below.)

**Portfolio links** are ONE link, the site itself, and the reader clicks into whichever gallery they want (Dan, 2026-07-30, #1832). This paragraph used to say the opposite, that a performance's discipline maps to the closest of the five site galleries (Music, Performing Arts, Bands, Comedy, Dance), and that rule is retired: a deep link into one of them is a choice made on the reader's behalf. It is not merely no longer advised, it is REFUSED: `DraftCheck.galleryPathLink` blocks a body carrying any of those five paths, and `DraftCheck.allowedLinkHost` allows only `danwrightphotography.com`, so any other host is something the drafter invented (#789).

**Pricing in the email:** Dan charges a flat 250 dollars per hour plus tax (waived for tax-exempt clients with documentation), one hour minimum, charging only for time at the performance. The gallery is delivered within two weeks. He does not discount, with one deliberate exception: he will compromise on rate for dance shoots, to build his nearly-empty dance portfolio. The cold email does not lead with that flexibility, but dance prospects are flagged as rate-flexible so Dan can offer it in negotiation. The drafter never promises freebies; concessions are Dan's to make later. **A COLD PITCH carries no rate and no turnaround at all (#1906, Dan's call 2026-07-31),** reversing the earlier rule stated here that every email states the rate plainly: a number in a first email from a stranger is judged before the work is. **A REPLY to someone who asks answers in full, in Dan's own two paragraphs, VERBATIM (#2874).** Neither this section nor any other summary is the source of that answer: the paragraphs live in the `dan-wright-brand-voice` skill (authoritative) and in `docs/prep-runbook.md` §2, the two the drift check compares. The compressed version above is background for a reader of this plan and must never be drafted from, which is exactly the mistake #2874 fixed: a summary of the answer is all a drafter can send.
...
- **Phase 2:** org scout plus venue scout plus the Claude travel and routing estimate. (This line also named discipline gallery links; retired with the rule above, #1832.)
