...
"Likely uncovered"
    `UI/QueueView+Model.swift`
"Link a reply from another thread"
    `Domain/LinkReplyFromAnotherThread.swift`
"Link their reply"
    `Domain/ProposedConversation.swift`
...
"Looks booked?"
    `UI/InquiryRowView.swift`
"Looks like the same show as \(other), already stored for this night."
    `UI/QueueView+Model.swift`
"Looks right"
    `UI/ProspectRowView.swift`
...
"New listings. The scout is reading them now."
    `Domain/SourceReadState.swift`
"No AI draft started, because Overture couldn't hand this reply to the drafter: \(error.localizedDescription)"
    `Domain/ReplyPanel.swift`
"No AI draft started, because this conversation isn't waiting on an answer from you."
    `Domain/ReplyPanel.swift`
"No AI draft started. Another reply is already being drafted, so try again once it finishes."
    `Domain/ReplyPanel.swift`
"No AI draft started. \(reason)"
    `Domain/ReplyPanel.swift`
...
"That contact"
    `App/ActionFeedback.swift`
"That contact is no longer on \(org), so nothing was changed. It was merged into another contact, or struck off"
    `Domain/ReachedOutSnapshot.swift`
"That contact is no longer on the show, so nothing was changed. It was merged into another contact, or struck off"
    `Domain/ReachedOutSnapshot.swift`
...
"\(org) closed out: you turned them down."
    `Domain/ShowOutcome.swift`
"\(org) closed: its nights are covered elsewhere."
    `Domain/ShowOutcome.swift`
