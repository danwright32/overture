...
| `title-renames.json` | App (`ScoutService.apply`, through `TitleRenameLedger.record`, after the save that persisted the rename and never before) | `scripts/what-renamed-a-title.sh`, by hand, when a stored row has been renamed onto another show and which mechanism did it has to be established (#4147, the question #4068 could not answer) | none (one object, `entries`, each holding `key`, `from`, `to`, `arm`, `at`; a reader ignores any field it does not know, so an arm name a newer build writes decodes in an older one) | `fixtures/title-renames/` | `TitleRenameLedgerTests.swift`, `scripts/what-renamed-a-title.test.sh` |
