...
struct SourceYieldTests {
    ...
    // The line Dan reads, kept-first: "3 of 12 kept", with sent and booked appended only when they are
    // above zero (his choice), so the common case stays quiet.
    @Test func theLineLeadsWithTheKeptRatio() {
        var t = SourceYield.Tally(found: 12, kept: 3, approved: 2, sent: 1, booked: 1)
        #expect(SourceYield.line(t) == "3 of 12 kept · 1 sent · 1 booked")

        t = SourceYield.Tally(found: 4, kept: 2, approved: 0, sent: 0, booked: 0)
        #expect(SourceYield.line(t) == "2 of 4 kept")

        t = SourceYield.Tally(found: 5, kept: 3, approved: 2, sent: 2, booked: 0)
        #expect(SourceYield.line(t) == "3 of 5 kept · 2 sent")
    }

    // Dead weight: found many, kept none. The line says so plainly rather than staying silent, because
    // making this visible is the entire point of the feature.
    @Test func deadWeightReadsZeroOfMany() {
        let t = SourceYield.Tally(found: 12, kept: 0, approved: 0, sent: 0, booked: 0)
    ...
    @Test func aSourceThatFoundNothingSaysNothing() {
        let t = SourceYield.Tally(found: 0, kept: 0, approved: 0, sent: 0, booked: 0)
