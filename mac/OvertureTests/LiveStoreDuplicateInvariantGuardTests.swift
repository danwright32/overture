...
struct LiveStoreDuplicateInvariantGuardTests {
    ...
    @Test func theReplayHelperRunsTheRealLaunchSequence() {
        ...
        // inside a test (L2).
        // #3774: isolation is now spelled `ScratchDefaults.make`, the only way a test may build a defaults
        // suite, and the only one that also deletes the suite's file afterwards.
        let isolatesSettings = source.contains("ScratchDefaults.make(")
