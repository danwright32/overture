...
struct StandDownDueListTests {
    ...
    @Test func theDueCountDropsWithTheRow() throws {
        ...
        #expect(DueWork.counts(prospects: [p], inquiries: [], now: now, replyRunAlive: false).followUps == 1)

        p.recipients.first!.standDownOutreach(now: now)
        try? ctx.save()

        #expect(DueWork.counts(prospects: [p], inquiries: [], now: now, replyRunAlive: false).followUps == 0)
