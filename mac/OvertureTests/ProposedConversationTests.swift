...
struct ProposedConversationTests {
    ...
    func aProposalCountsAsDue() throws {
        ...
        let p = show(ctx)
        let r = formPitch(ctx, on: p)
        let before = DueWork.counts(prospects: [p], inquiries: [], now: now, replyRunAlive: false)
        ProposedConversation.propose(candidate("m1"), on: r, now: now)

        let after = DueWork.counts(prospects: [p], inquiries: [], now: now, replyRunAlive: false)
    ...
    func aDeclinedProposalStopsCounting() throws {
        ...
        #expect(DueWork.counts(prospects: [p], inquiries: [], now: now, replyRunAlive: false).conversationsToConfirm == 0)
    }
