...
enum ClientTagCopy {
    ...
    static func stateLabel(isClient: Bool, override: Bool?, namedClient: String? = nil) -> String? {
        switch override {
        case .some(true):
            // #1358: an "always" tag can name WHICH Downbeat client (the shared-venue case, where the
            // source name is the venue, not the client). When it does, say which, so the row shows the
            // named client the coverage diagnostic now counts as covered. A bare "always" keeps the
            // unnamed wording.
            if let namedClient {
                return "Tagged as returning client \(namedClient): shows surface up to a year ahead."
            }
            return "Tagged a returning client: shows surface up to a year ahead."
    ...
    // #1358: under the "Always" submenu, the choice to tag the source a returning client without naming a
    // specific Downbeat client (the bare tag: arms the year-ahead horizon, but the coverage diagnostic
    // cannot attribute it to one client). Sits above the per-client list.
    static let optionAlwaysNoClient = "Without naming a client"
}
